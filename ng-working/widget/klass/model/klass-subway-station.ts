...
export class KlassSubwayStation {

	// selectile에서 선택할 수 있는 선택지를 보여주는 리스트.
	public selectableList: KlassSubwayStation[];
	// selectile에서 선택된 idx
	...
	getKeyNotDefault():string {
		if(this.myArray.isOK(this.selectableList)) {
			// 기본값 여부 검사
			let target:KlassSubwayStation = this.selectableList[0];
	...
	isSame(target:KlassSubwayStation):boolean {
		return this.myIs.isSame(this, target);
	}	

	isSharing(key:string, target:KlassSubwayStation):boolean {
