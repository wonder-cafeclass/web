...
export class KlassSubwayLine {

	// selectile에서 선택할 수 있는 선택지를 보여주는 리스트.
	public selectableList: KlassSubwayLine[];
	// selectile에서 선택된 idx
	...
	getKeyNotDefault():string {
		if(this.myArray.isOK(this.selectableList)) {
			// 기본값 여부 검사
			let target:KlassSubwayLine = this.selectableList[0];
	...
	isSame(target:KlassSubwayLine):boolean {
		return this.myIs.isSame(this, target);
	}	

	isSharing(key:string, target:KlassSubwayLine):boolean {
