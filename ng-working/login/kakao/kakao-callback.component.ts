...
export class KakaoCallbackComponent implements AfterViewInit, OnDestroy {
  ...
  ngAfterViewInit(): void {
    ...
    if(this.isDebug()) console.log("kakao-callback / ngAfterViewInit");
    this.asyncViewPack();
  ...
  private asyncViewPack(): void {
    ...
    if(this.isDebug()) console.log("kakao-callback / asyncViewPack / 시작");

    // 이미 View 기본정보가 들어왔다면 바로 가져온다.
    if(this.watchTower.getIsViewPackReady()) {
      if(this.isDebug()) console.log("kakao-callback / asyncViewPack / isViewPackReady : ",true);
      this.init();
    } // end if
    ...
    this.watchTower.isViewPackReady$.subscribe(
      ...
      if(this.isDebug()) console.log("kakao-callback / asyncViewPack / subscribe / isViewPackReady : ",isViewPackReady);
  ...
  private init(): void {
    ...
    if(this.isDebug()) console.log("kakao-callback / init / 시작");
  ...
  private logActionPage() :void {
    ...
    if(this.isDebug()) console.log("kakao-callback / logActionPage / 시작");
    ...
    this.subscription = this.activatedRoute.queryParams.subscribe(
      (param: any) => {
        ...
        if(null != this.code && "" != this.code) {
          if(this.isDebug()) console.log("kakao-callback / getQueryString / this.code : ",this.code);
  ...
  private getKakaoToken(kakaoCode:string) :void{
    ...
    if(this.isDebug()) console.log("kakao-callback / getKakaoToken / 시작");
    if(this.isDebug()) console.log("kakao-callback / getKakaoToken / kakaoCode : ",kakaoCode);

    if(null == kakaoCode || "" == kakaoCode) {
      if(this.isDebug()) console.log("kakao-callback / getKakaoToken / 중단 / kakaoCode is not valid!");
    ...
    .then((myResponse:MyResponse) => {
      ...
      if(this.isDebug()) console.log("kakao-callback / getKakaoToken / myResponse : ",myResponse);

      let accessToken:string = myResponse.getDataProp("access_token");
      let tokenType:string = myResponse.getDataProp("token_type");

      if(this.isDebug()) console.log(`kakao-callback / getKakaoToken / accessToken : ${accessToken}`);
      if(this.isDebug()) console.log(`kakao-callback / getKakaoToken / tokenType : ${tokenType}`);

      if( myResponse.isSuccess() && 
          null != accessToken && 
          null != tokenType) {

        if(this.isDebug()) console.log(`kakao-callback / getKakaoToken / 유저 앱등록을 진행합니다.`);
        ...
        this.myLoggerService.logError(
          ...
          this.myLoggerService.errorAPIFailed,
          // errorMsg:string
          `kakao-callback / getUserByKakaoId / kakaoCode : ${kakaoCode}`
        ); // end logger      
  ...
  private getKakaoSignUp(kakaoTokenType:string, kakaoAccessToken:string) :void {
    ...
    if(this.isDebug()) console.log("kakao-callback / getKakaoSignUp / 시작");

    this.loginService
    .getKakaoSignUp(kakaoTokenType, kakaoAccessToken)
    .then((myResponse:MyResponse) => {

      if(this.isDebug()) console.log("kakao-callback / getKakaoSignUp / myResponse : ",myResponse);
      ...
      if( myResponse.isSuccess() && 
        ...
        if(this.kakaoSignupCodeAlreadyRegisterd === code) {
          ...
          if(this.isDebug()) console.log("kakao-callback / getKakaoSignUp / code : ",code);
          if(this.isDebug()) console.log("kakao-callback / getKakaoSignUp / msg : ",msg);
        ...
        this.myLoggerService.logError(
          ...
          this.myLoggerService.errorAPIFailed,
          // errorMsg:string
          `kakao-callback / getKakaoSignUp / kakaoTokenType : ${kakaoTokenType} / kakaoAccessToken : ${kakaoAccessToken}`
        ); // end logger      
  ...
  private logError(errorType:string, errMsg:string) :void {
    ...
    if(this.isDebug()) console.log("kakao-callback / logError / 시작");
    ...
    this.myLoggerService.logError(
      ...
      if(this.isDebug()) console.log("kakao-callback / logError / myResponse : ",myResponse);
  ...
  private getKakaoMe(kakaoTokenType:string, kakaoAccessToken:string) :void {
    ...
    if(this.isDebug()) console.log("kakao-callback / getKakaoMe / 시작");

    this.loginService
    .getKakaoMe(kakaoTokenType, kakaoAccessToken)
    .then((myResponse:MyResponse) => {

      if(this.isDebug()) console.log("kakao-callback / getKakaoMe / myResponse : ",myResponse);
      ...
      if( myResponse.isSuccess() && !(0 < kakaoId) ) {
        ...
        // 카카오 플랫폼에서의 로그인 성공!
        if(this.isDebug()) console.log(`kakao-callback / getKakaoMe / kakao_id : ${kakaoId}`);
        ...
        this.myLoggerService.logError(
          ...
          this.myLoggerService.errorAPIFailed,
          // errorMsg:string
          `kakao-callback / getKakaoMe / kakaoTokenType : ${kakaoTokenType} / kakaoAccessToken : ${kakaoAccessToken}`
        ); // end logger      
  ...
  private getUserByKakaoId(kakaoId:string) :void {
    ...
    if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / 시작");

    this.userService
    .getUserByKakaoId(kakaoId)
    .then((myResponse:MyResponse) => {

      if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / myResponse : ",myResponse);
      ...
      if(myResponse.isSuccess() && null == user) {
        ...
        if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / 중단 / result is not valid!");
        // 에러 로그 등록
        this.myLoggerService.logError(
          // apiKey:string
          this.watchTower.getApiKey(),
        ...
        // 하지만 추가 정보 필요. 
        // 회원 가입창으로 이동.
        if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / 중단 / 회원 가입창으로 이동.");
        ...
        if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / 로그인 성공!, 로그인 쿠키 만듦");

        // 가져온 유저 정보를 전파.
        let userFromDB = myResponse.getDataProp("user");
        if(null != userFromDB) {
          let user:User = new User().setJSON(userFromDB);
          if(this.isDebug()) console.log(`kakao-callback / getUserByKakaoId / user : `,user);
          ...
          .then((myResponse:MyResponse) => {
            ...
            if(this.isDebug()) console.log(`kakao-callback / getTeacher / myResponse : `,myResponse);
        ...
        .confirmUserKakao(
          ...
          if(myResponse.isSuccess()) {
            ...
            // 쿠키 인증 성공!
            if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / 쿠키 인증 성공! 홈으로 이동.");
            ...
            if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / redirectUrl : ",redirectUrl);
            ...
            // kakaoid로 쿠키 인증 실패. 
            if(this.isDebug()) console.log("kakao-callback / getUserByKakaoId / 중단 / kakaoid로 쿠키 인증 실패. 홈으로 이동.");
        ...
        this.myLoggerService.logError(
          ...
          this.myLoggerService.errorAPIFailed,
          // errorMsg:string
          `kakao-callback / getUserByKakaoId / kakaoId : ${kakaoId}`
        ); // end logger      
