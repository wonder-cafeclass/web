...
var SignupComponent = (function () {
    ...
    SignupComponent.prototype.updateUser = function () {
        ...
        if (this.userCopy.isNotOK()) {
            if (this.isDebug())
                console.log("signup / updateUser / 중단 / this.userCopy.isNotOK()");
            return;
        } // end method
        this.userService
            .updateUser(
    ...
    SignupComponent.prototype.addUser = function () {
        ...
        if (this.isDebug())
            console.log("signup / addUser / this.user.id : ", this.userCopy.id);
        if (this.isDebug())
            console.log("signup / addUser / this.email : ", this.userCopy.email);
        if (this.isDebug())
            console.log("signup / addUser / this.password : ", this.userCopy.password);
        if (this.isDebug())
            console.log("signup / addUser / this.name : ", this.userCopy.name);
        if (this.isDebug())
            console.log("signup / addUser / this.nickname : ", this.userCopy.nickname);
        if (this.isDebug())
            console.log("signup / addUser / this.gender : ", this.userCopy.gender);
        if (this.isDebug())
            console.log("signup / addUser / this.birthYear : ", this.userCopy.getBirthYear());
        if (this.isDebug())
            console.log("signup / addUser / this.birthMonth : ", this.userCopy.getBirthMonth());
        if (this.isDebug())
            console.log("signup / addUser / this.birthDay : ", this.userCopy.getBirthDay());
        if (this.isDebug())
            console.log("signup / addUser / this.thumbnail : ", this.userCopy.thumbnail);
        if (this.isDebug())
            console.log("signup / addUser / this.mobileNumHead : ", this.userCopy.getMobileHead());
        if (this.isDebug())
            console.log("signup / addUser / this.mobileNumBody : ", this.userCopy.getMobileBody());
        if (this.isDebug())
            console.log("signup / addUser / this.mobileNumTail : ", this.userCopy.getMobileTail());
        if (this.userCopy.isNotOK()) {
            if (this.isDebug())
                console.log("signup / addUser / 중단 / this.userCopy.isNotOK()");
            return;
        } // end method
        this.userService
            .addUser(
    ...
    SignupComponent.prototype.checkEmailUnique = function (email) {
        ...
        if (null == email || "" === email) {
            if (this.isDebug())
                console.log("signup / checkEmailUnique / 중단 / email is not valid!");
            return;
        }
        else if (null == this.user) {
            if (this.isDebug())
                console.log("signup / checkEmailUnique / 중단 / this.user is not valid!");
            return;
        }
        else if (null == this.userCopy) {
            if (this.isDebug())
                console.log("signup / checkEmailUnique / 중단 / this.userCopy is not valid!");
            return;
        }
        ...
        if (null != this.user.email &&
            ...
            if (this.isDebug())
                console.log("signup / checkEmailUnique / 중단 / this.user.email === this.userCopy.email");
            return;
        } // end if
        // DB Unique test
        this.userService
            ...
            if (myResponse.isSuccess() &&
                ...
                if (_this.isDebug())
                    console.log("signup / checkEmailUnique / user : ", user);
                if (null == user) {
                    // 해당 이메일로 등록된 유저는 없습니다. 
                    // email 등록이 가능합니다.
                    if (_this.isDebug())
                        console.log("signup / checkEmailUnique / 해당 이메일로 등록된 유저는 없습니다. email 등록이 가능합니다.");
                    _this.userCopy.email = email;
                    _this.userCopy.isDuplicatedEmail = false;
                    _this.emailComponent.hideWarningTooptip();
                    if (_this.isDebug())
                    ...
                    // 경고 창을 노출합니다.
                    if (_this.isDebug())
                        console.log("signup / checkEmailUnique / 해당 이메일로 이미 등록된 유저가 있습니다. 경고 창을 노출합니다.");
                    _this.emailComponent.showTooltipFailWarning(
                    // msg:string,
                    "이메일이 이미 등록되어 있습니다.", 
                    // isTimeout:Boolean
                    false);
                    _this.userCopy.isDuplicatedEmail = true;
                } // end if
            } // end if
        }); // end service
    }; // end method
    SignupComponent.prototype.checkMobileUnique = function (mobile) {
        ...
        if (null == mobile || "" === mobile) {
            if (this.isDebug())
                console.log("signup / checkMobileUnique / 중단 / mobile is not valid!");
            return;
        }
        else if (null == this.user) {
            if (this.isDebug())
                console.log("signup / checkMobileUnique / 중단 / this.user is not valid!");
            return;
        }
        else if (null == this.userCopy) {
            if (this.isDebug())
                console.log("signup / checkMobileUnique / 중단 / this.userCopy is not valid!");
            return;
        }
        ...
        if (null != this.user.mobile &&
            ...
            if (this.isDebug())
                console.log("signup / checkMobileUnique / 중단 / this.user.mobile === mobile!");
            return;
        }
        // 휴대전화 번호가 모두 확인되었습니다. 
        // DB Unique test
        ...
        this.userCopy.getMobileTail())
            ...
            if (myResponse.isSuccess()) {
                ...
                if (null != userJSON) {
                    _this.userCopy.isDuplicatedMobile = true;
                    ...
                    _this.userCopy.isDuplicatedMobile = false;
                    _this.mobileComponent.hideTooltipBody(0);
                } // end method
            }
            else {
                // TODO - Error Report
